...
void display(unsigned char display_value)
{
    ...
    // Runs 51 times (0 to 50) with 10ms delay per digit for a 1 second hold time (51 * 20ms = 1.02s)
...
void main(void)
{
    ...
    while(1)
    {
        // Loop from 0x01 up to 0xFF, incrementing by 2 (odd numbers only)
        // Since we are using an unsigned char, 0xFF + 2 = 0x01 (overflows)
        for (count = 0x01; ; count += 2)
        {
            display(count);

            // Break the loop after 0xFF to restart at 0x01 (via the for loop initializer)
            if (count == 0xFF)
            {
                break; 
            }
